...
export class VerificationManager<TUser extends IUser> extends EventEmitter {
    ...
    #validateOptions(options: VerificationOptions<TUser>) {
        ...
        if (
            ...
            !options.codeGenerationOptions.length
        ) {
            options.codeGenerationOptions = {
                ...(options.codeGenerationOptions ?? {}),
                length: 6,
        ...
        if (!options.errorMessage) {
            options.errorMessage = (user: TUser) =>
                `An error occured when trying to send something to ${user?.username}!`;
        }
        ...
        if (!options.alreadyActiveMessage) {
            options.alreadyActiveMessage = () =>
                "An account is already verified with this data!";
        }
        if (!options.validCodeMessage) {
            options.validCodeMessage = (user: TUser, validCode: string) =>
                `Your code ${validCode} is valid! Welcome ${user.username}!`;
        }
        if (!options.invalidCodeMessage) {
            options.invalidCodeMessage = (_, invalidCode: string) =>
                `Your code ${invalidCode} is invalid!`;
        }
    }
     ...
     * @memberof VerificationManager
     */
    // biome-ignore lint: data could be anything
    public async sendCode(userid: Snowflake, data: any): Promise<string> {
        let user: TUser | undefined | null =
            await this.#storingSystem.read(userid);
        if (!user)
            user = await this.#storingSystem.readBy(new Map([["data", data]]));
        ...
        if (!user) {
            ...
            if (
                user.nbCodeCalled %
                    this.#options.maxNbCodeCalledBeforeResend ===
                0
            ) {
                try {
                    await this.#senderAPI.send({
    ...
    public async verifyCode(userid: string, code: string): Promise<string> {
        ...
        const user: TUser | undefined | null =
            await this.#storingSystem.read(userid);
        ...
        if (user) {
            ...
            isVerified =
                user.status === UserStatus.pending && user.code === code.trim();
            if (isVerified) {
